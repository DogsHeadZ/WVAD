...
def eval_epoch(config, model, test_dataloader):
    ...
    with torch.no_grad():
        ...
        for i, (frames, flows, labels) in enumerate(test_dataloader):
            ...
            ref_scores, nor_scores, abn_scores, scores= model(frames, frames, frames)  # 他这个注意力机制一次要输入一整个视频，而不能一帧一帧进行检测

            # logits = torch.squeeze(ref_scores, 2)[0]
            logits = ref_scores[0]
...
def train(config):
    ...
    for step in tqdm(
        ...
        with torch.set_grad_enabled(True):
            model.train()
            ref_scores, nor_scores, abn_scores, scores = model(ref_rgb_nor, norm_frames, abn_frames)  # b*32  x 2048
            ...
            ref_scores, nor_scores, abn_scores, scores = model(ref_rgb_abn, norm_frames, abn_frames)  # b*32  x 2048
            loss_criterion = RTFM_loss(0.0001, 100)  # 这里就只用到了topk个帧进行分类损失
