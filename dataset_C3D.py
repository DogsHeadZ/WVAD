...
class Test_Dataset_SHT_C3D(Dataset):
    def __init__(self,rgb_h5_file, flow_h5_file,test_txt,test_mask_dir,
                 segment_len=16,ten_crop=False,height=128,width=171,crop_size=112):
        ...
        self.rgb_transforms = transforms.Compose([ transforms.Resize([self.height, self.width]),
    ...
    def frame_processing(self, frames):
        ...
        for i, frame in enumerate(frames):
            new_frames.append(cv2.imdecode(np.frombuffer(frame, np.uint8), cv2.IMREAD_COLOR))  # 这里和I3D有点不一样
...
class Train_TemAug_Dataset_SHT_C3D(Dataset):
    def __init__(self, rgb_h5_file, flow_h5_file,train_txt, pseudo_labels,clip_num=8,segment_len=16,
        ...
        self.rgb_transforms=transforms.Compose([transforms.Resize((128,171)),
